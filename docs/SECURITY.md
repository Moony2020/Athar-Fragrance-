...
- Stage 5.4 Wishlist entries are Product-level canonical slugs only. The `athar_guest_wishlist` cookie is opaque, httpOnly, SameSite=Lax, Path=/, and Secure in production; no price, inventory, account identity, or client-owned Wishlist payload is stored. Every mutation revalidates public Product eligibility server-side. Durable/live Atlas Wishlist persistence remains unverified.
