...
export function CartPage({ cart }: { cart: PublicCart }) {
  ...
  if (cart.lines.length === 0) return <section className={styles.empty} aria-labelledby="cart-title"><h1 id="cart-title">Your bag is empty</h1><p>Discover a fragrance to make it yours.</p><Button className={styles.explore} href="/shop">Explore fragrances <span aria-hidden="true">↗</span></Button></section>;
  return <section className={styles.page} aria-labelledby="cart-title">
    ...
    <header className={styles.heading}><div><h1 id="cart-title">Your bag</h1></div><span>{cart.totalQuantity} {cart.totalQuantity === 1 ? "item" : "items"}</span></header>
